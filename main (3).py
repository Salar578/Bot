...
class Bot(BaseBot):
    ...
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        await self.highrise.chat(f"درود خوش آمدی دوست عزیز✨💗!،  @{user.username}  برای ایجاد احساسات 1 تا 96 را وارد کنید ، برای احساسات تصادفی بنویسید دنس و برای توقف کردن بنویسید stop .")
        try:
    ...
    async def on_user_leave(self, user: User):
        ...
        user_id = user.id
        farewell_message = f"خدانگهدار دوست عزیز زود برگرد 🥹! @{user.username}"
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("kiss bot"):
        ...
        teleport_locations = {
            "vip": Position(15.5, 16.0, 17.),
            "kat1": Position(13.5, 11.0, 7.5),
            "/f2": Position(13., 7.25, 7.),
        ...
        elif message.lower().startswith(" stop kick") and await self.is_user_allowed(user):
            target_username = message.split("@")[-1].strip()
            room_users = await self.highrise.get_room_users()
        ...
        if                          message.lower().startswith("/goto") or message.lower().startswith("tp"):
          target_username =         message.split("@")[-1].strip()
        ...
        if message.lower().startswith("!summon") and await self.is_user_allowed(user):
            parts = message.split()
        ...
        if message.lower().startswith("mojazat") and await self.is_user_allowed(user):
        ...
        if message.lower() == "stop" and await self.is_user_allowed(user):
            if self.following_user is not None:
                await self.highrise.chat("من دنبال کردن را متوقف کردم.")
                self.following_user = None
            else:
                await self.highrise.chat("من در حال حاضر کسی را دنبال نمی کنم.")
        ...
        if message == "stop" or message == "stop" or message == "0":
            if user_id in self.user_emote_loops:
                await self.stop_emote_loop(user_id)
        ...
        try:
            if message.lstrip().startswith(("fly")):
                response = await self.highrise.get_room_users()
                users = [content[0] for content in response.content]
                usernames = [user.username.lower() for user in users]
        ...
        if message.startswith("دنس") or message.startswith("دنس"):
            try:
                emote_name = random.choice(list(secili_emote.keys()))
                emote_to_send = secili_emote[emote_name]["value"]
                await self.highrise.send_emote(emote_to_send, user.id)
            except:
                print("Dans emote bir hata oluştu.")
...
#Ulti Ulti Ulti Ulti Ulti Ulti Ulti
  ...
  #Genel Genel Genel Genel Genel
    ...
    async def is_user_allowed(self, user: User) -> bool:
        ...
        return user_privileges.moderator or user.username in ["the.pirate","Amin0097","Rahaa25"]
...
#gellllbbb
    ...
    async def userinfo(self, user: User, target_username: str) -> None:
        ...
        if not user_info.users:
            await self.highrise.chat("User not found, please tell us a valid user")
            return
...
class RunBot():
  room_id = "6575bb7d902269d10e00ae06"
  bot_token = "5de4410c6fda6d9033c8f372d028f77d86e6d4354e1fc0d10ae9ff3349a761d6"
  bot_file = "main"
  bot_class = "Bot"
